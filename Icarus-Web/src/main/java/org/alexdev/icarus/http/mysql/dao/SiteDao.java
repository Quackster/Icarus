...
public class SiteDao {
    ...
    public static void updateKey(SiteKey key, String value) {
        ...
        try {
            ...
            preparedStatement = Storage.get().prepare("UPDATE `site_config` SET `value` = ? WHERE `key` = ?", sqlConnection);
    ...
    public static boolean keyExists(SiteKey key) {
        ...
        try {
            ...
            sqlConnection = Storage.get().getConnection();
            preparedStatement = Storage.get().prepare("SELECT * FROM `site_config` WHERE `key` = ?", sqlConnection);
    ...
    public static String get(SiteKey key) {
        ...
        try {
            ...
            sqlConnection = Storage.get().getConnection();
            preparedStatement = Storage.get().prepare("SELECT * FROM `site_config` WHERE `key` = ? LIMIT 1", sqlConnection);
            preparedStatement.setString(1, key.getKey());
            resultSet = preparedStatement.executeQuery();
